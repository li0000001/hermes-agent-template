...
ENV_VARS = [
    ...
    ("S3_BUCKET_NAME",           "S3 bucket",                "storage",   False),
    ("S3_PREFIX",                "S3 key prefix",            "storage",   False),
    ("AWS_REGION",               "AWS region",               "storage",   False),
]
...
def write_env(path: Path, data: dict[str, str]) -> None:
    ...
    cat_order = ["model", "provider", "tool", "storage",
                 "telegram", "discord", "slack", "whatsapp",
                 "email", "mattermost", "matrix", "gateway"]
    cat_labels = {
        "model": "Model", "provider": "Providers", "tool": "Tools", "storage": "Storage",
...
# S3 persistence — restores config on boot, pushes on every write.
# No-op if S3_BUCKET_NAME is not set.
s3_sync = S3Sync(hermes_home=HERMES_HOME)
...
async def api_config_put(request: Request):
    ...
    try:
        ...
        asyncio.get_event_loop().run_in_executor(None, s3_sync.sync_up)
